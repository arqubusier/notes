...
- High speed data is clocked at 480.00Mb/s with a data signalling tolerance of Â± 500ppm.
- Full speed data is clocked at 12.000Mb/s with a data signalling tolerance of Â±0.25% or 2,500ppm.
- Low speed data is clocked at 1.50Mb/s with a data signalling tolerance of Â±1.5% or 15,000ppm.
...
The rule of thumb in this case is the compiler picks the candidate function whose parameters match the arguments most closely is the one that is called.
...
variadic functions are the most generic
...
A templated function can actually be more precise than a normal function.
However, in case of a draw, the normal function will have the precedence. 
...
**** Outside vs. Inside class
Outside allows you to define overloads like so
T1 operatorX(T2, MyClass), where the lhs term may be something other than the class in question.
...
*** Type Erasure
Use different types throud a single generic interface.
**** *void in C
**** *polymorphism in C++ (OO)
**** Templates in C++
Similarities to duck-typing.
*** TODO SFINAE
...
When trying to substitute template parameter types to instantiate templates,
a failure will not lead to a compiler error.

However this does not apply to all cases of substitution.

a rule-of-thumb is that substitution outside of function bodys are safe.

**** C++98
use sizeof operator for "fake evaluation" at compile time

**** C++11
decltype operator gives you the type of the of the expression it will evaluate.

    B b;
    decltype(b.serialize()) test = "test"; // Evaluate b.serialize(), which is typed as std::string.
    // Equivalent to std::string test = "test";

declval utility, used to give a "fake reference" to an object.
    struct Default {
...
auto parameters in lambda -> generate template function in lambda operator().
...
*** Negative Testing
Also known as failure testing or error-path testing.
* Computer Science
** IEEE 754
*** TODO lookup
https://www.youtube.com/watch?v=k12BJGSc2Nc
exponent shift-127 encoded
std::nextafter
*** Denormalized (subnormal)
    
https://en.wikipedia.org/wiki/Denormal_number
In a normal floating-point value, there are no leading zeros in the significand;
rather, leading zeros are removed by adjusting the exponent (for example, the number 0.0123 would be written as 1.23 × 10−2).
Denormal numbers are numbers where this representation would result in an exponent that is below the smallest representable exponent (the exponent usually having a limited range). 
Such numbers are represented using leading zeros in the significand. 

 A floating-point number may be recognized as denormal whenever its exponent is the least value possible. For binary IEEE 745 exponent = 0 indicates subnormal.

*** Normalized
    
1 < exponent < exponent max

*** Special
inf: exponent == max  and mantissa == 0
NaN: exponent == max and maintisa != 0


** String to floating point
https://legacy.cs.indiana.edu/~dyb/pubs/FP-Printing-PLDI96.pdf

v+ successive floating point number
v- successive floating point number

Shortest rounding path? round-trip?
rounding strategies

https://www.researchgate.net/publication/2295884_How_to_Read_Floating_Point_Numbers_Accurately

Can use floating point arithmetic for many cases:
Use single floating point multiplication/division to scale input
then test if the error is sufficiently small.
If not we have to resort to a special algorithm that uses arbitrarily big integers.
** 
* Misc Tools
** org-mode
** organice
...
* Debugging, Benchmarking, Inspection
** sourcetrail
...
** Map files
Can be produced by linker, for example gcc's linker.
Shows, among other things addresses and sizes of entities in the executable sections.

Add the following flags to gcc to generate a linker map file.
